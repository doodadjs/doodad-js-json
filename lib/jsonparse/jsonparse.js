//*********************************************************************************
// Source: https://gist.github.com/creationix/1821394
// Referer: https://github.com/creationix/jsonparse
// Author: Tim Caswell, CreationIX
// License: ???????? - Taken from a Gist, without any mention of the license.
//
// Changes :
//       1) Author: Claude Petit
//          Description: Adapted for both client/server
//       2) Author: Claude Petit
//          Description: Added "onStartString" and "onEndString", and no longer cummulates strings between chunks (to prevent DoS)
//*********************************************************************************
...
(function(exports) {
	...
	var proto = JsonParser.prototype;
	proto.charError = function (buffer, i) {
	  this.callbacks.onError(new Error("Unexpected " + JSON.stringify(buffer[i]) + " at position " + i + " in state " + toknam(this.state)));
	};
	proto.parse = function (buffer) {
	  ...
	  for (var i = 0, l = buffer.length; i < l; i++) {
		...
		case NUMBER8: // * After digit (after +/-)
		  ...
		  continue;
		case TRUE1: // r
		  if (buffer.charCodeAt(i) === 0x72) {
			this.state = TRUE2;
			continue;
		  }
		  this.charError(buffer, i);
		case TRUE2: // u
		  if (buffer.charCodeAt(i) === 0x75) {
			this.state = TRUE3;
			continue;
		  }
		  this.charError(buffer, i);
		case TRUE3: // e
		  if (buffer.charCodeAt(i) === 0x65) {
			this.state = START;
		  ...
		  this.charError(buffer, i);
		case FALSE1: // a
		  if (buffer.charCodeAt(i) === 0x61) {
			this.state = FALSE2;
			continue;
		  }
		  this.charError(buffer, i);
		case FALSE2: // l
		  if (buffer.charCodeAt(i) === 0x6c) {
			this.state = FALSE3;
			continue;
		  }
		  this.charError(buffer, i);
		case FALSE3: // s
		  if (buffer.charCodeAt(i) === 0x73) {
			this.state = FALSE4;
			continue;
		  }
		  this.charError(buffer, i);
